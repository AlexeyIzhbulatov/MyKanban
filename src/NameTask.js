...
const left = (<svg width="1em" height="1em" viewBox="0 0 16 16" className="bi bi-arrow-left-short" fill="currentColor"
  ...
  <path fillRule="evenodd"
    ...
    const colorAlert = ['alert alert-primary', 'alert alert-success', 'alert alert-danger']

  return (
    <div className="nameTask">
        <div className={colorAlert[props.tasks.priority]}>
            <div className="card-header"></div>
            <div className="card-body">
                <h5 className="card-title">{props.tasks.name}</h5>
                <p className="card-text">Some quick example text to build on the card title and make up the bulk of the
                    card's content.</p>
                <span onClick={() => props.changeStatus({ id: props.tasks.id, direction: 'left'})}>{left}</span>
                <span onClick={() => props.changeStatus({ id: props.tasks.id, direction: 'right'})}>{right}</span>
                <span onClick={() => props.changeStatus({id: props.tasks.id, direction: 'up'})}>{up}</span>
